...
// FUNGSI MEMPERSINGKAT FETCH FOOTBAL-DATA.ORG
const fetchFootball = endpoint=>{
...
// FUNGSI UNTUK MENYISIPKAN LOADER
const insertLoader = (elmnt, position='beforeend')=>{
    if(elmnt)
        elmnt.insertAdjacentHTML(position, `
            <div class="progress">
...
const premierLeague = async ()=>{
    ...
    try{
        const response = await fetchFootball('/competitions/2021/standings')
...
const championsLeague = async ()=>{
    ...
    try{
        const response = await fetchFootball('/competitions/2001/standings');
        const resJson  = await response.json();
...
const addFavorite = db =>{
    ...
    if(type === 'player'){
        data.id = id.player;
        addData = playerOS.put(data);
    }

    if(type === 'team'){
        data.id = id.team;
        addData = teamOS.put(data);
    }
